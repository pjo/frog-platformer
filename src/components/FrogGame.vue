...
watch(uiScale, (s) => document.documentElement.style.setProperty('--ui-scale', String(s)), {
  immediate: true,
});
...
const {
  ...
  stopBgMusic,
  closeAudioCtx,
} = useAudio(muted, paused, gameOver, won);
...
const leaderboard = computed<LeaderEntry[]>(() => {
  const base = loadingScores.value
    ? []
    : isOnline.value
      ? [...remoteScores.value]
      : [...BASE_SCORES];
...
const stateBridge = {
  ...
  get levelBannerTimer() {
    return levelBannerTimer;
  },
  set levelBannerTimer(v) {
    levelBannerTimer = v;
  },
  get currentLevel() {
  ...
  get fliesCollected() {
    return fliesCollected.value;
  },
  set fliesCollected(v) {
    fliesCollected.value = v;
  },
  get lives() {
    return lives.value;
  },
...
function advanceLevel() {
  score.value +=
    SCORING.LEVEL_CLEAR_BASE +
    lives.value * SCORING.LEVEL_CLEAR_LIFE_BONUS +
    fliesCollected.value * SCORING.LEVEL_CLEAR_FLY_BONUS;
  sfxWin();
...
function startGame(d: 'easy' | 'normal' | 'hard') {
  engine?.input.blockCurrentKeys();
  clearInputFrames = 3;
  difficulty.value = d;
...
function loseLife() {
  ...
  player.x = checkpoint.x;
  player.y = PLAYER_CFG.RESPAWN_Y;
  player.vx = 0;
  player.vy = PLAYER_CFG.RESPAWN_VY;
  player.speedTimer = 0;
  player.starTimer = 0;
  activePower.value = '';
  world.cameraX = Math.max(
    0,
    Math.min(player.x - VIEWPORT_W * ENGINE.CAMERA_OFFSET_RATIO, world.width - VIEWPORT_W),
  );
}
  ...
  >
    ...
    <main class="stage card" ref="stageRef">
      ...
      <StartScreen
        ...
        @update:playerName="playerName = $event"
        @update:playerEmail="playerEmail = $event"
        @start="startGame"
      />

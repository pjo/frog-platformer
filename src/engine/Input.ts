export class InputManager {
  ...
  get left() {
    return (
      (this.keys.has('ArrowLeft') && !this.blocked.has('ArrowLeft')) ||
      (this.keys.has('KeyA') && !this.blocked.has('KeyA'))
    );
  }
  get right() {
    return (
      (this.keys.has('ArrowRight') && !this.blocked.has('ArrowRight')) ||
      (this.keys.has('KeyD') && !this.blocked.has('KeyD'))
    );
  }
  get jump() {
    return (
      (this.keys.has('Space') && !this.blocked.has('Space')) ||
      (this.keys.has('ArrowUp') && !this.blocked.has('ArrowUp')) ||
      (this.keys.has('KeyW') && !this.blocked.has('KeyW'))
    );
  }
}

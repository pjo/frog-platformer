...
export class Renderer {
  ...
  private rr(x: number, y: number, w: number, h: number, r: number) {
    ...
    this.ctx.lineTo(x, y+r); this.ctx.quadraticCurveTo(x, y, x+r, y); this.ctx.closePath(); this.ctx.fill()
  }
  ...
  private drawParallax(state: GameState) {
    ...
    theme.drawParallax(this.ctx, state.world.cameraX, state.world.width, ENGINE.VIEWPORT_W, ENGINE.VIEWPORT_H)
  }

  private drawWorld(state: GameState, ui: UIState) {
    this.ctx.save(); this.ctx.translate(-state.world.cameraX, 0)
  ...
  private drawGround(state: GameState) {
    const theme = THEMES[LEVELS[state.currentLevel - 1].theme]
    this.ctx.fillStyle = theme.groundColor; this.ctx.fillRect(0, 560, state.world.width, 160)
    ...
    for (let i = 0; i < grassCount; i++) {
      ...
      this.ctx.beginPath(); this.ctx.moveTo(x,560); this.ctx.lineTo(x+10,530); this.ctx.lineTo(x+18,560); this.ctx.closePath(); this.ctx.fill()
    }
  }
  ...
  private drawPlatforms(state: GameState) {
    for (const p of state.platforms) {
      ...
      if (p.type === 'stone') {
        this.ctx.fillStyle = COLORS.PLATFORM_STONE; this.rr(p.x, p.y, p.w, p.h, 8)
        this.ctx.fillStyle = COLORS.PLATFORM_STONE_HIGHLIGHT; this.rr(p.x+4, p.y+4, p.w-8, 6, 4)
      } else {
        this.ctx.fillStyle = COLORS.PLATFORM_WOOD; this.rr(p.x, p.y, p.w, p.h, 10)
        this.ctx.fillStyle = COLORS.PLATFORM_WOOD_PATTERN
        for (let i = 0; i < p.w; i+=28) this.ctx.fillRect(p.x+i, p.y+4, 16, p.h-8)
      }
      if (p.vx) {
        this.ctx.fillStyle = COLORS.PLATFORM_MOVING_HIGHLIGHT; this.rr(p.x, p.y, p.w, p.h, 10)
      }
    }
  }
  ...
  private drawCheckpoints(state: GameState) {
    for (const cp of state.checkpoints) {
      this.ctx.fillStyle = COLORS.CHECKPOINT_POLE; this.ctx.fillRect(cp.x+10, cp.y, 8, cp.h)
      const pulse = cp.active ? 1 + Math.sin(performance.now()*0.008)*0.06 : 1
      this.ctx.save(); this.ctx.translate(cp.x+18, cp.y+10); this.ctx.scale(pulse, pulse)
      this.ctx.fillStyle = cp.active ? COLORS.CHECKPOINT_ACTIVE : COLORS.CHECKPOINT_INACTIVE
      this.ctx.beginPath(); this.ctx.moveTo(0,0); this.ctx.lineTo(46,14); this.ctx.lineTo(0,28); this.ctx.closePath(); this.ctx.fill()
      this.ctx.restore()
    }
  }
  ...
  private drawPowerUps(state: GameState) {
    for (const pu of state.powerUps) {
      ...
      this.ctx.save(); this.ctx.translate(pu.x, pu.y+bob); this.ctx.scale(pulse, pulse)
      if (pu.type === 'speed') {
        this.ctx.fillStyle = COLORS.PU_SPEED_BASE; this.ctx.beginPath(); this.ctx.arc(0,0,pu.r,0,Math.PI*2); this.ctx.fill()
        this.ctx.fillStyle = COLORS.PU_SPEED_GLINT; this.ctx.beginPath(); this.ctx.arc(-4,-4,5,0,Math.PI*2); this.ctx.fill(); this.ctx.beginPath(); this.ctx.arc(4,-2,4,0,Math.PI*2); this.ctx.fill()
        this.ctx.fillStyle = COLORS.PU_SPEED_TEXT; this.ctx.font = 'bold 10px sans-serif'; this.ctx.textAlign = 'center'; this.ctx.fillText('S', 0, 4)
      } else {
        this.ctx.fillStyle = COLORS.PU_STAR_BASE; this.drawStarShape(0, 0, pu.r, pu.r*0.45, 5)
        this.ctx.fillStyle = COLORS.PU_STAR_TEXT; this.ctx.font = 'bold 8px sans-serif'; this.ctx.textAlign = 'center'; this.ctx.fillText('\u2605', 0, 3)
      }
      this.ctx.restore()
    }
  }
  ...
  private drawHazards(state: GameState) {
    for (const h of state.hazards) {
      ...
      for (let i = 0; i < h.w; i+=18) {
        ...
        this.ctx.beginPath(); this.ctx.arc(h.x+i+8, h.y+8+Math.sin((i+performance.now()*0.02)*0.2)*2, 4, 0, Math.PI*2); this.ctx.fill()
      }
    }
  }
  ...
  private drawEnemies(state: GameState) {
    for (const e of state.enemies) {
      if (e.dying) {
        ...
        this.ctx.save(); this.ctx.globalAlpha = Math.max(0, 1-t)
        this.ctx.translate(e.x + e.w/2, e.y + e.h); this.ctx.scale(1 + t*1.4, Math.max(0.05, 1 - t*0.95))
        if (e.isBoss) {
          this.ctx.fillStyle = COLORS.PARTICLE_BOSS_DEATH; this.rr(-e.w/2, -12, e.w, 12, 4)
        } else if (this.spriteSheet) {
          this.ctx.imageSmoothingEnabled = false
          this.ctx.drawImage(this.spriteSheet, 0, SPRITE.rows.smurfWalk*SPRITE.fh, SPRITE.fw, SPRITE.fh, -22, -38, 44, 38)
        }
  ...
  private drawBoss(e: Enemy, yb: number) {
    ...
    this.ctx.beginPath(); this.ctx.moveTo(-30,-34); this.ctx.lineTo(-22,-56); this.ctx.lineTo(-8,-38); this.ctx.lineTo(0,-58); this.ctx.lineTo(8,-38); this.ctx.lineTo(22,-56); this.ctx.lineTo(30,-34); this.ctx.closePath(); this.ctx.fill()
    this.ctx.fillStyle = COLORS.TEXT_PRIMARY; this.ctx.beginPath(); this.ctx.arc(-15,-10,10,0,Math.PI*2); this.ctx.fill(); this.ctx.beginPath(); this.ctx.arc(15,-10,10,0,Math.PI*2); this.ctx.fill()
    this.ctx.fillStyle = COLORS.PARTICLE_DAMAGE;  this.ctx.beginPath(); this.ctx.arc(-15,-10, 6,0,Math.PI*2); this.ctx.fill(); this.ctx.beginPath(); this.ctx.arc(15,-10, 6,0,Math.PI*2); this.ctx.fill()
    this.ctx.fillStyle = COLORS.TEXT_PRIMARY;  this.ctx.beginPath(); this.ctx.arc(-13,-12, 2,0,Math.PI*2); this.ctx.fill(); this.ctx.beginPath(); this.ctx.arc(17,-12, 2,0,Math.PI*2); this.ctx.fill()
    this.ctx.strokeStyle = COLORS.BOSS_EYE_PUPIL; this.ctx.lineWidth = 3
    this.ctx.beginPath(); this.ctx.arc(0, 10, 20, 0, Math.PI); this.ctx.stroke()
    this.ctx.fillStyle = COLORS.BOSS_HP_BG_DARKEN; this.ctx.fillRect(-36,-52,72,9)
    this.ctx.fillStyle = e.hp===3 ? COLORS.BOSS_HP_HIGH : e.hp===2 ? COLORS.BOSS_HP_MED : COLORS.BOSS_HP_LOW
    this.ctx.fillRect(-36,-52, 72*(e.hp/e.maxHp), 9)
    this.ctx.restore()
  }

  private drawFinishGate(state: GameState) {
    const bossLocked = state.currentLevel === LEVELS.length && state.enemies.some(e => e.isBoss && e.alive)
    ...
    this.ctx.fillRect(state.finishGate.x+state.finishGate.w-10, state.finishGate.y, 10, state.finishGate.h)
    ...
    this.ctx.save(); this.ctx.translate(state.finishGate.x+state.finishGate.w/2, state.finishGate.y+18); this.ctx.scale(pulse, pulse)
    this.ctx.fillStyle = bossLocked ? COLORS.PARTICLE_DAMAGE : COLORS.PARTICLE_BOSS_DEATH
    this.rr(-36, 0, 72, 34, 10)
    this.ctx.fillStyle = COLORS.TEXT_PRIMARY; this.ctx.font = 'bold 18px sans-serif'; this.ctx.textAlign = 'center'
    this.ctx.fillText(bossLocked ? 'BOSS!' : state.currentLevel < LEVELS.length ? `LVL ${state.currentLevel+1}` : 'EXIT', 0, 24)
    this.ctx.restore()
  }
  ...
  private drawPlayer(state: GameState, ui: UIState) {
    ...
    else if (!state.player.onGround)           row = SPRITE.rows.frogJump,   frame = Math.min(5, Math.floor(Math.max(-12, Math.min(12, state.player.vy+12))/4))
  ...
  private drawParticlesInWorld(state: GameState, ui: UIState) {
    ...
    for (const p of state.particles) {
      this.ctx.fillStyle = p.color; this.ctx.globalAlpha = p.life/p.maxLife
  ...
  private drawOverlay(state: GameState, ui: UIState) {
    ...
    this.ctx.font = '800 64px sans-serif'; this.ctx.textAlign = 'center'
    
    if      (ui.won)      this.ctx.fillText('Victory!',  ENGINE.VIEWPORT_W/2, ENGINE.VIEWPORT_H/2-20)
    else if (ui.gameOver) this.ctx.fillText('Game Over', ENGINE.VIEWPORT_W/2, ENGINE.VIEWPORT_H/2-20)
    else                  this.ctx.fillText('Paused',    ENGINE.VIEWPORT_W/2, ENGINE.VIEWPORT_H/2-20)
    
    this.ctx.font = '24px sans-serif'
    const sub = ui.won      ? `Final score ${ui.score} \u00B7 ${ui.fliesCollected}/${ui.totalFlies} flies \u00B7 All levels cleared!`
              : ui.gameOver ? 'Press R to try again.'
  ...
  private drawLevelBanner(state: GameState) {
    ...
    this.ctx.fillStyle = COLORS.OVERLAY_BG; this.ctx.fillRect(0, ENGINE.VIEWPORT_H/2-52, ENGINE.VIEWPORT_W, 104)
    this.ctx.fillStyle = COLORS.TEXT_PRIMARY; this.ctx.font = 'bold 40px sans-serif'; this.ctx.textAlign = 'center'
    ...
    this.ctx.fillText(`Level ${state.currentLevel}: ${names[state.currentLevel-1]}`, ENGINE.VIEWPORT_W/2, ENGINE.VIEWPORT_H/2+14)
    this.ctx.restore()
  }
}

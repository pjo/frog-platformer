...
function makeEnemy(overrides: Partial<Enemy> = {}): Enemy {
  return {
    x: 400, y: 522, w: 44, h: 38,
    vx: 2, vy: 0, patrolVx: 2, minX: 300, maxX: 600,
    alive: true, dying: false, deathTimer: 0,
    bob: 0, hue: 0, mood: 'walk',
...
function makeNoInput(): InputManager { return new InputManager() }
...
describe('Boss phase thresholds', () => {
  it('applies 1.8x speed multiplier in AGGRESSIVE phase', () => {
    const state = makeState({
      isBoss: true, hp: 3, maxHp: 5,  // ratio 0.6, below PHASE_AGGRESSIVE (0.66)
      patrolVx: 2, vx: 2, vy: 0,
    })
  ...
  it('applies 2.8x speed multiplier in BERSERK phase', () => {
    const state = makeState({
      isBoss: true, hp: 1, maxHp: 5,  // ratio 0.2, below PHASE_BERSERK (0.33)
      patrolVx: 2, vx: 2, vy: 0, onGround: false,
    })
    state.player.x = 9999  // far away
    Physics.update(state, makeNoInput(), 16.6667, makeEvents(), true)
...
describe('resetEntities field reset', () => {
  it('resets shield, cooldowns, and vy after respawn', () => {
    const enemy = makeEnemy({ type: 'shield', shieldUp: false, shieldCooldown: 45, jumpCooldown: 20, shootCooldown: 30, vy: 7, onGround: false })

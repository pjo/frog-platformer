...
function drawIceParallax(ctx: CanvasRenderingContext2D, cameraX: number, worldWidth: number, vw: number, vh: number) {
  const sky = ctx.createLinearGradient(0, 0, 0, vh)
  ...
  for (let i = 0; i < 7; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(mx, 480); ctx.lineTo(mx + 120, 260); ctx.lineTo(mx + 240, 480); ctx.closePath(); ctx.fill()
    ctx.fillStyle = '#e0f2fe'
    ctx.beginPath(); ctx.moveTo(mx + 120, 260); ctx.lineTo(mx + 95, 320); ctx.lineTo(mx + 145, 320); ctx.closePath(); ctx.fill()
  }
  ...
  for (let i = 0; i < 30; i++) {
    ...
    ctx.beginPath(); ctx.arc(fx, fy, 2 + i % 2, 0, Math.PI * 2); ctx.fill()
  }
}

function drawDesertParallax(ctx: CanvasRenderingContext2D, cameraX: number, worldWidth: number, vw: number, vh: number) {
  const sky = ctx.createLinearGradient(0, 0, 0, vh)
  sky.addColorStop(0, '#fbbf24'); sky.addColorStop(1, '#fb923c')
  ctx.fillStyle = sky; ctx.fillRect(0, 0, vw, vh)

  // Sun with glow
  ctx.save()
  ctx.fillStyle = 'rgba(254,240,138,0.3)'; ctx.beginPath(); ctx.arc(1100, 100, 90, 0, Math.PI * 2); ctx.fill()
  ctx.fillStyle = 'rgba(253,224,71,0.5)';  ctx.beginPath(); ctx.arc(1100, 100, 65, 0, Math.PI * 2); ctx.fill()
  ctx.fillStyle = '#fde047';               ctx.beginPath(); ctx.arc(1100, 100, 44, 0, Math.PI * 2); ctx.fill()
  ctx.restore()
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(dx, 560); ctx.bezierCurveTo(dx + 80, 420, dx + 240, 420, dx + 320, 560); ctx.closePath(); ctx.fill()
  }
...
function drawJungleParallax(ctx: CanvasRenderingContext2D, cameraX: number, worldWidth: number, vw: number, vh: number) {
  const sky = ctx.createLinearGradient(0, 0, 0, vh)
  sky.addColorStop(0, '#14532d'); sky.addColorStop(1, '#15803d')
  ctx.fillStyle = sky; ctx.fillRect(0, 0, vw, vh)
  ...
  for (let i = 0; i < 8; i++) {
    ...
    ctx.beginPath(); ctx.arc(tx + 40, 180, 70, 0, Math.PI * 2); ctx.arc(tx + 80, 160, 60, 0, Math.PI * 2); ctx.arc(tx + 15, 200, 55, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#15803d'
    ctx.beginPath(); ctx.arc(tx + 50, 175, 55, 0, Math.PI * 2); ctx.arc(tx + 85, 155, 48, 0, Math.PI * 2); ctx.fill()
  }
  ...
  for (let i = 0; i < 9; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(vx2, 0); ctx.quadraticCurveTo(vx2 + 20, vLen / 2, vx2, vLen); ctx.stroke()
    ctx.fillStyle = '#22c55e'
    ctx.beginPath(); ctx.ellipse(vx2, vLen, 10, 6, 0.3, 0, Math.PI * 2); ctx.fill()
    ctx.beginPath(); ctx.ellipse(vx2 + 4, vLen - 30, 8, 5, -0.3, 0, Math.PI * 2); ctx.fill()
  }
...
function drawUnderwaterParallax(ctx: CanvasRenderingContext2D, cameraX: number, worldWidth: number, vw: number, vh: number) {
  const sky = ctx.createLinearGradient(0, 0, 0, vh)
  sky.addColorStop(0, '#0c4a6e'); sky.addColorStop(1, '#0284c7')
  ctx.fillStyle = sky; ctx.fillRect(0, 0, vw, vh)
  ...
  for (let i = 0; i < 6; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(rx, 0); ctx.lineTo(rx + 60, 0); ctx.lineTo(rx + 120, vh); ctx.lineTo(rx - 40, vh); ctx.closePath(); ctx.fill()
  }
  ctx.restore()
  ...
  for (let i = 0; i < 10; i++) {
    ...
    ctx.strokeStyle = i % 2 === 0 ? '#15803d' : '#0ea5e9'; ctx.lineWidth = 4
    ctx.beginPath(); ctx.moveTo(sx, 560)
    for (let s = 0; s < 5; s++) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    ctx.strokeStyle = 'rgba(186,230,253,0.5)'; ctx.lineWidth = 1.5
    ctx.beginPath(); ctx.arc(bx, by, 4 + i % 4, 0, Math.PI * 2); ctx.stroke()
  }
}

function drawVoidParallax(ctx: CanvasRenderingContext2D, cameraX: number, worldWidth: number, vw: number, vh: number) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.strokeStyle = `hsla(${270 + i * 30},100%,70%,${alpha})`; ctx.lineWidth = 3
  ...
  for (let i = 0; i < 7; i++) {
    ...
    ctx.strokeStyle = `hsla(${280 + i * 20},100%,65%,0.4)`; ctx.lineWidth = 2
...
export const THEMES: Record<string, ThemeRenderer> = {
  ...
  ice:       { drawParallax: drawIceParallax,       groundColor: '#93c5fd', groundAccentA: '#bfdbfe', groundAccentB: '#60a5fa' },
  desert:    { drawParallax: drawDesertParallax,    groundColor: '#b45309', groundAccentA: '#d97706', groundAccentB: '#92400e' },
  jungle:    { drawParallax: drawJungleParallax,    groundColor: '#15803d', groundAccentA: '#22c55e', groundAccentB: '#166534' },
  underwater:{ drawParallax: drawUnderwaterParallax,groundColor: '#0369a1', groundAccentA: '#0ea5e9', groundAccentB: '#075985' },
  void:      { drawParallax: drawVoidParallax,      groundColor: '#4c1d95', groundAccentA: '#7c3aed', groundAccentB: '#2e1065' },
}
